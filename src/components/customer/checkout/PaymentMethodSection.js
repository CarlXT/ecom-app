...
export function PaymentMethodSection({ formData, onChange, onNext }) {
  ...
  return html`
    <div className="space-y-6">
      ...
      <p className="text-zinc-400 text-xs mb-6">Select your preferred payment option.</p>

      <div className="space-y-3">
        ${paymentMethods.map((method) => html`
          <label key=${method} className="flex items-center justify-between p-4 bg-[#1c1c20] border ${formData.paymentMethod === method ? 'border-red-500' : 'border-zinc-700/60'} rounded-xl cursor-pointer hover:border-zinc-500">
