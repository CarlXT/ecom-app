...
export function FilledButton({ children, onClick, className = '' }) {
  return html`
    <button
      type="button"
      onClick=${onClick}
      className=${`px-8 py-3 bg-[#E50914] hover:bg-red-700 active:scale-95 text-white font-extrabold text-sm sm:text-base tracking-wider uppercase rounded-full shadow-lg shadow-red-900/40 transition-all duration-200 focus:outline-none cursor-pointer ${className}`}
    >
      ${children}
    </button>
  `;
...
export default function BannerSection({ 
  ...
  return html`
    ...
    >
      ...
      <div className="absolute inset-0 p-8 sm:p-12 md:p-16 flex flex-col justify-end z-10">
        <div className="max-w-xl space-y-3 sm:space-y-4">
          
          <!-- Tag Line -->
          <span className="block text-white text-base sm:text-xl font-bold tracking-widest uppercase opacity-95 drop-shadow-md">
            ${currentProduct.tag}
          </span>

          <!-- Product Title -->
          <h2 className="text-white text-3xl sm:text-5xl md:text-6xl font-black uppercase tracking-tight leading-[1.05] drop-shadow-lg">
