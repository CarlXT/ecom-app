...
export function ProductsPage({
  // Accept categories list dynamically via props
  categories = ['Headphones', 'Microphones', 'Filters', 'Mounts'],
  selectedCategory,
  onSelectCategory,
  onAddCategory
}) {
  // Local active state fallback if controlled state is not passed
  const [activeCategory, setActiveCategory] = useState(selectedCategory || categories[0]);
  
  // State for Create Category Modal visibility
  ...
  return html`
    <div className="w-full min-h-screen bg-[#0a0a0c] text-white flex flex-col font-['SF_Pro',-apple-system,BlinkMacSystemFont,sans-serif]">
      ...
      <div className="sticky top-0 z-20 bg-[#0a0a0c]/95 backdrop-blur-md px-6 lg:px-12 pt-8 pb-6 border-b border-white/10 flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 pt-2">
          ...
          <!-- Horizontal Scrollable Dynamic Pills using OutlinedButton -->
